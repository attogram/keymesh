...
export async function deriveAllAddresses(mnemonic) {
  ...
  for (const chain of ['bitcoin', 'litecoin', 'dogecoin']) {
    ...
    let payment;
    if (chain === 'dogecoin') {
        payment = bitcoin.payments.p2pkh({ pubkey: child.publicKey, network: networks[chain] });
    } else {
        payment = bitcoin.payments.p2wpkh({ pubkey: child.publicKey, network: networks[chain] });
    }
