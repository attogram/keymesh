...
program
    .name('keymesh')
    .description('A CLI tool to generate a mesh of cryptocurrency addresses or convert address formats.')
    .version('1.0.0');
    ...
    .action(async (seed_phrase) => {
        try {
            ...
            for (const [chain, address] of Object.entries(addresses)) {
                const chainName = chain.replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase());
...
program
    ...
    .description('Convert a cryptocurrency address to other native formats (Feature in development).')
    .action((address) => {
